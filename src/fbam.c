...
void append_log(FILE* log_file, time_t time, char* function_name, char* file_path, size_t count, off_t offset)
{
    ...
    snprintf(log_message, MAX_LOG_LEN, "{\"timestamp\": \"%s\", \"function_name\": \"%s\", \"file_path\": \"%s\", \"size\": \"%zu\", \"offset\": \"%ld\"}\n",
             format_timestamp(time_buffer, time), function_name, file_path, count, offset);
